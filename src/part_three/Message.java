...
public class Message {
    ...
    public static void searchByRecipient(String number) {
        StringBuilder sb = new StringBuilder("Messages for " + number + ":\n\n");

        for (String msg : sentMessages) {
            if (msg.contains(number)) sb.append(msg).append("\n\n");
        }

        if (sb.toString().equals("Messages for " + number + ":\n\n")) {
    ...
    public static void deleteByHash(String hash) {
        ...
        for (int i = 0; i < sentMessages.size(); i++) {
            if (sentMessages.get(i).contains(hash)) {
